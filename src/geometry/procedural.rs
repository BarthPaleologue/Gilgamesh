...
impl ProceduralMesh {
    ...
    pub fn terrain(name: &str, size: f32, nb_subdivisions: u32, height_fn: &dyn Fn(f32, f32) -> f32, engine: &Engine) -> Mesh {
        ...
        mesh.set_material(PhongMaterial::new(&format!("{}Material", name), &engine.wgpu_context));
    ...
    /// `engine` is a mutable reference to the Gilgamesh engine.
    pub fn sphere(name: &str, diameter: f32, nb_subdivisions: u32, height_fn: &dyn Fn(f32, f32, f32) -> f32, engine: &Engine) -> Mesh {
